...
export function LevelCard({
  ...
  return (
    <div
      className={`relative w-full rounded-2xl border-2 p-6 cursor-pointer transition-all duration-200 ${
        selected
          ...
          : isLocked
            ? kidMode
              ? 'bg-transparent border-surface-700 cursor-not-allowed opacity-60'
              : 'bg-surface-800/80 border-surface-600 cursor-not-allowed opacity-60'
            : isCurrentLevel
              ? kidMode
                ? 'bg-transparent border-yellow-400 shadow-lg shadow-yellow-400/30 hover:shadow-yellow-400/40'
    ...
    >
      ...
      {isLocked && (
        <div
          className={`absolute inset-0 rounded-2xl flex items-center justify-center ${kidMode ? 'bg-black/30' : 'bg-black/50'}`}
        >
